...
## 대규모 재평가 (n=50, 2026-06-18 추가)

18-seed 측정의 신뢰도를 높이기 위해 **LIBERO 표준인 50회 trial**로 재평가하였다.
씬은 동일(움직이는 산닭 8마리 + 정지 폐사체 1마리, 폐사 비율 ≈ 11%, 학습분포 평가).

**자산 상태(중요):** 94%를 기록한 DAgger 체인 최종 모델(r4)의 병합 체크포인트는 디스크
정리/복구 사고로 소실되었고, 체인의 **원본 r1이 삭제**되어 어댑터 재병합으로는 충실히
복원되지 않는다(재병합 시 base 불일치로 붕괴 — 원본 성공 seed들에서 grasp 0/3 확인).
따라서 대규모 재평가는 **재현 가능한 작동 모델 r1recov**(2a-30000 + DAgger r1 재학습
어댑터)로 수행하였다.

| 평가 | n | task 성공 | grasp 성공 | 투하 정밀도(중앙값) |
|---|---|---|---|---|
| r4 (피크, 모델 소실) | 18 | 94.4% | 94% | 1.95 cm |
| r1recov | 18 | 83.3% | 89% | — |
| **r1recov (대규모)** | **50** | **90.0%** | **92.0%** | **1.34 cm** |

- 투하 정밀도(성공 n=45): 중앙값 **1.34 cm**, 평균 1.47, 최대 3.93 cm, **전부 바구니 중심 ≤5 cm(100%)**.
- 실패 5건(seed 12005·12021·12024·12039·12051)은 모두 grasp 단계 실패.
- 결과 로그: `eval_wd_sweep/r1recov_50seed.jsonl`.
- **함의:** 재현 가능한 작동 모델이 50회에서 **task 90.0%**로 안정 수렴 — 소표본(83.3%)
  대비 향상되었고 헤드라인 94%(n=18, 소실 모델)와 4.4%p 차. 논문 대표 수치로는
  재현 가능한 **90% (n=50)** 권장, 피크 94%(n=18) 병기 가능.
