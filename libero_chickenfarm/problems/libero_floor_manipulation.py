...
class Libero_Floor_Manipulation(BDDLBaseDomain):
    def __init__(self, bddl_file_name, *args, **kwargs):
        ...
        # 양계장(chicken farm) task는 갈색 흙 floor의 poultry scene 사용.
        # BDDL 파일명/경로 또는 BDDL 내용(problem name)에 "chicken_farm"/"poultry" 포함 시 자동 적용.
        bddl_path = (bddl_file_name or "").lower()
        is_poultry = ("chicken_farm" in bddl_path) or ("poultry" in bddl_path)
        if not is_poultry and bddl_file_name and os.path.isfile(bddl_file_name):
            try:
                with open(bddl_file_name, "r") as _f:
                    _head = _f.read(2048).lower()
                is_poultry = (
                    "chicken_farm" in _head
                    or "poultry" in _head
                    or "blue chick" in _head
                    or "dead chick" in _head
                    or "- chicken" in _head
                    or "- chick_crouching" in _head
                    or "- chick_looking_left" in _head
                    or "- dead_chick" in _head
                )
            except OSError:
                pass
        ...
        if "scene_xml" not in kwargs or kwargs["scene_xml"] is None:
            kwargs.update({
                "scene_xml": "scenes/libero_floor_poultry_style.xml"
                             if is_poultry else "scenes/libero_floor_base_style.xml"
            })
        if "scene_properties" not in kwargs or kwargs["scene_properties"] is None:
            # IR(적외선) 시뮬: dark floor + dark blue wall = 차가운 배경.
            # 살아있는 chick(warm)과 죽은 chick(cold blue)이 색 대비로 명확히 구분.
            kwargs.update(
                {
                    "scene_properties": {
                        "floor_style": "dark" if is_poultry else "meeka-beige",
                        "wall_style": "dark-blue" if is_poultry else "light-gray-plaster",
                    }
                }
            )
