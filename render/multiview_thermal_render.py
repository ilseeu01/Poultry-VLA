#!/usr/bin/env python
"""다중 뷰 열화상 렌더 — r4 solo 정책을 agentview/sideview/wrist 3뷰로 동시 캡처,
각 뷰에 thermal_fx 적용 후 가로 타일 mp4. 결과 시연용(여러 뷰 IR 영상).

사용 (oft env, GPU):
  BLUE_CHICK_THERMAL=1 MUJOCO_GL=osmesa python multiview_thermal_render.py \
      --ckpt <merged_r4> --seeds 12001,12004,12009 \
      --out-dir /home/capstone/openvla/rollouts/2026_06_14_multiview
"""
import os
...
VIEWS = ["agentview", "sideview", "robot0_eye_in_hand"]
VIEW_LABEL = {"agentview": "AGENTVIEW", "sideview": "SIDEVIEW", "robot0_eye_in_hand": "WRIST"}
...
def tile_views(obs):
    """3뷰 thermal → 가로 타일 1프레임."""
    tiles = []
    for v in VIEWS:
        key = v + "_image"
        if key not in obs:
            continue
        img = thermal_fx(np.ascontiguousarray(obs[key][::-1]), colorbar=True)
...
def main():
    ...
    ap.add_argument("--post-done-steps", type=int, default=0,
                    help="env done 이후에도 이만큼 더 롤아웃(안착·정착 프레임 확보; 모델 정지 dummy)")
    ap.add_argument("--trace", action="store_true", help="병아리 z·바구니거리·그리퍼 trace 출력")
    args = ap.parse_args()
    os.makedirs(args.out_dir, exist_ok=True)
    seeds = [int(x) for x in args.seeds.split(",")]

    policy = VLANavPolicy(args.ckpt, args.unnorm_key)
    print(f"[mv] model loaded; seeds={seeds}")

    for seed in seeds:
        name = f"_mv_{seed}.bddl"
        if not gen_bddl(seed, name, 1):
            print(f"[mv] seed {seed} bddl gen fail")
            continue
        bddl = os.path.join(get_libero_path("bddl_files"), "libero_object", name)
        env = None
        try:
            env = OffScreenRenderEnv(
                bddl_file_name=bddl, camera_names=VIEWS,
            ...
            for t in range(args.max_steps):
                ...
                frames.append(tile_views(obs))
                if args.trace and t % 8 == 0:
                    p = C.get_body_pos(env, dead[0])
                    d = float(np.hypot(p[0] - basket[0], p[1] - basket[1]))
                    print(f"    [trace t={t}] chick z={p[2]:.3f} d_basket={d:.3f} "
                          f"grip={act[-1]:+.0f} done={done}")
                if term or done:
                    done_at = t
                    break
            # env done 이후 추가 롤아웃(모델 정지) — 안착·정착 프레임 확보
            ...
            print(f"[mv] seed {seed}: done={done}@{done_at} chick_final z={pend[2]:.3f} "
                  f"d_basket={dend:.3f}")
            out = os.path.join(args.out_dir, f"mv_seed{seed}_{tag}.mp4")
            w = imageio.get_writer(out, fps=30)
            for f in frames:
                w.append_data(f)
            w.close()
            print(f"[mv] seed {seed}: {len(frames)} frames done={done} lift={lift*100:.1f}cm → {out}")
        except Exception as e:
            print(f"[mv] seed {seed} SKIP: {type(e).__name__}: {e}")
        finally:
            if env is not None:
                try:
                    env.close()
